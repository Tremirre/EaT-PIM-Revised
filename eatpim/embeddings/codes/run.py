...
def parse_args(args=None):
    ...
    parser.add_argument(
        "--do_example",
        action="store_true",
        help="show an example ingredient prediction",
    )
    parser.add_argument(
    ...
    with open(file_path) as fin:
        for line in fin:
            ...
            if pathlen not in triples:
                triples[pathlen] = []
            triples[pathlen].append(
                (
                    tuple(entity2id[str(h_content)] for h_content in eval(h)),
...
def main(args):
    ...
    args.save_path = os.path.join(
        path.DATA_DIR, args.data_path, args.save_path
    )  # (path.DATA_DIR / args.data_path / args.save_path).resolve()

    if args.init_checkpoint:
        args.init_checkpoint = os.path.join(
            path.DATA_DIR, args.data_path, args.init_checkpoint
        )
    ...
    data_dir = (path.DATA_DIR / args.data_path / "triple_data").resolve()
    ...
    ingredient_ids = [
        entity2id[ing] for ing in ingredient_list if ing in entity2id.keys()
    ]
    ...
    train_graphs = read_graphs(
        (data_dir / "train.txt").resolve(), entity2id, relation2id
    )
    logging.info("#train: %d" % len(train_graphs))
    valid_graphs = read_graphs(
        (data_dir / "valid.txt").resolve(), entity2id, relation2id
    )
    ...
    train_triples = read_triple(
        (data_dir / "trip_train.txt").resolve(), entity2id, relation2id
    )
    logging.info("#train triples: %d" % len(train_triples))
    valid_triples = read_triple(
        (data_dir / "trip_valid.txt").resolve(), entity2id, relation2id
    )
    logging.info("#valid triples: %d" % len(valid_triples))
    test_triples = read_triple(
        (data_dir / "trip_test.txt").resolve(), entity2id, relation2id
    )
    ...
    if args.do_train:
        ...
        triple_train_iterator = BidirectionalOneShotIterator(
            triple_train_dataloader_head, triple_train_dataloader_tail
        )
        ...
        for step in range(init_step, args.max_steps):
            if step % train_triples_every_n == 0 and step > 0:
                ...
                log = kge_model.train_step(
                    kge_model, optimizer, triple_train_iterator, args
                )
            else:
                log = kge_model.train_step(
                    kge_model, optimizer, graph_train_iterator, args
                )
